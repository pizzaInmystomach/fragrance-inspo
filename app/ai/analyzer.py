...
from .prompts import INPUT_PARSING_PROMPT, CHARACTER_PROMPT, MATCHING_PROMPT, DESCRIPTION_PROMPT, TRAIT_ENHANCEMENT_PROMPT
...
class CharacterAnalyzer:
    ...
    def _extract_json_from_text(self, text):
                ...
                if field in cleaned_text:
                    ...
                    if field_start != -1:
                        ...
                        if value_start != -1:
                            ...
                            if closing_brace != -1 and (closing_quote == -1 or closing_quote > closing_brace):
                                # 需要添加結束引號
                                insert_pos = value_start + 1 + closing_brace
                                cleaned_text = cleaned_text[:insert_pos] + '"' + cleaned_text[insert_pos:]
                                break
        ...
        if '{' in cleaned_text and ('"traits"' in cleaned_text or '"status"' in cleaned_text):
            try:
                ...
                if json_part.count('{') > json_part.count('}'):
                    ...
                    for line in lines:
                        ...
                        elif line and ':' in line and (line.endswith(',') or line.endswith('"')):
                            fixed_lines.append(line)
    ...
    def parse_user_input(self, user_input):
        ...
        try:
            ...
            # 使用 LLM 進行智能解析
            content = self._safe_llm_call(self.input_parsing_chain, {
    ...
    def _quick_character_detection(self, user_input):
        ...
        character_map = {
            ...
            "elizabeth bennet": {"name": "Elizabeth Bennet", "source": "Pride and Prejudice"},
    ...
    def _fallback_input_parsing(self, user_input):
        ...
        greetings = ["hello", "hi", "hey", "good morning", "good evening", "how are you", 
                    "thanks", "thank you", "goodbye", "bye", "have a nice day"]
        ...
        descriptive_words = ["brave", "intelligent", "elegant", "mysterious", "strong", 
                           "smart", "beautiful", "confident", "kind", "powerful"]
        
        if any(word in input_lower for word in descriptive_words) and len(input_lower.split()) <= 3:
            return {
                "status": "need_clarification", 
                "character_name": None,
                "source": None,
    ...
    def analyze_character(self, character_name, source_type=""):
        ...
        try:
            ...
            content = self._safe_llm_call(self.character_chain, {
    ...
    def _get_default_character_analysis(self, character_name):
        ...
        character_traits_map = {
            ...
            "hermione granger": ["intelligent", "studious", "brave", "loyal", "perfectionist"],
            ...
            "daisy buchanan": ["elegant", "sophisticated", "charming", "mysterious", "alluring"],
            "奧黛麗": ["elegant", "graceful", "timeless", "sophisticated", "charming"],
            "audrey": ["elegant", "graceful", "timeless", "sophisticated", "charming"],
            "audrey hepburn": ["elegant", "graceful", "timeless", "sophisticated", "charming"],
            "james bond": ["sophisticated", "confident", "suave", "mysterious", "charming"],
    ...
    def enhance_fragrance_data(self, fragrance):
        ...
        try:
            ...
            # 使用LLM生成增强信息
            content = self._safe_llm_call(self.trait_enhancement_chain, {
                "fragrance_name": name,
                "brand": brand,
    ...
    def match_fragrances(self, character_analysis, fragrances, num_recommendations=3):
        ...
        try:
            ...
            print(f"🤖 開始 LLM 匹配分析...")
            content = self._safe_llm_call(matching_chain, {
            ...
            if content:
                ...
                if match_result and isinstance(match_result, dict) and "recommendations" in match_result:
                    print(f"✅ LLM 匹配成功")
                    return self._process_llm_recommendations(match_result, enhanced_fragrances, character_analysis)
            
            print(f"⚠️ LLM 匹配失敗，使用備用邏輯")
            return self._generate_smart_recommendations(enhanced_fragrances, character_analysis, num_recommendations)
            
        except Exception as e:
            print(f"❌ 香水匹配錯誤: {str(e)}")
            return self._generate_smart_recommendations(enhanced_fragrances if 'enhanced_fragrances' in locals() else [], character_analysis, num_recommendations)
    
    def _process_llm_recommendations(self, match_result, enhanced_fragrances, character_analysis):
        """處理 LLM 返回的推薦結果"""
        ...
        for i, rec in enumerate(recommendations):
            ...
            rationale = rec.get("rationale", "This fragrance matches the character's unique style.")
            
            # 尋找對應的香水
            matched_fragrance = next(
                (f for f in enhanced_fragrances if f["id"] == fragrance_id),
                enhanced_fragrances[i] if i < len(enhanced_fragrances) else enhanced_fragrances[0]
            )
    ...
    def _generate_smart_recommendations(self, enhanced_fragrances, character_analysis, num_recommendations):
        """生成智能推薦"""
        ...
        for fragrance in enhanced_fragrances:
            score = self._calculate_match_score(fragrance, character_traits, character_style)
    ...
    def _calculate_match_score(self, fragrance, character_traits, character_style):
        ...
        fragrance_traits = fragrance.get("additional_traits", []) + fragrance.get("personality_match", [])
    ...
    def _generate_smart_rationale(self, fragrance, character_analysis):
        ...
        if personality_match and traits:
            common_traits = [t for t in traits if any(t.lower() in pm.lower() for pm in personality_match)]
            if common_traits:
    ...
    def match_fragrance(self, character_analysis, fragrances):
        ...
        result = self.match_fragrances(character_analysis, fragrances, num_recommendations=1)
        if result and result.get("recommendations"):
            return result["recommendations"][0]
        return {
            "fragrance": self._create_basic_enhanced_fragrance(fragrances[0]) if fragrances else None,
